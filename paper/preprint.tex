...
\IEEEauthorblockA{School of Informatics\\University of Edinburgh\\Edinburgh, United Kingdom\\
...
% ===== ABSTRACT =====
\begin{abstract}
Bilateral teleoperation---where the operator feels forces from the remote environment---has historically required expensive force/torque sensors costing \$4,000--10,000 per joint.
Recent low-cost systems such as ALOHA and GELLO have made teleoperation dramatically more accessible, but operate in a strictly unilateral mode: the operator receives no haptic feedback.
We present a dual-pair bimanual teleoperation system that achieves bilateral force feedback using only motor current readings from commodity \$27.50 Dynamixel servos, eliminating the need for dedicated force sensors entirely.
Our system pairs two standard PincherX-100 leader arms with two custom-built 2$\times$-scaled follower arms featuring 3D-printed links and a torque-tiered motor configuration that reduces follower actuator cost by 55\%.
In a user study ($n = 22$), participants detected obstacles via force feedback with 91.1\% sensitivity, and rated the system's learnability at 7/7 (median).
The complete dual-pair system costs approximately \$3,000---an order of magnitude less than ALOHA.
We identify a fundamental limitation: motor current cannot distinguish gravitational loads from external contact forces without a dynamics model, establishing a clear boundary for sensorless bilateral control on commodity hardware.
All hardware designs, software, and CAD files are open-source.
\end{abstract}
...
For decades, this architecture required industrial-grade hardware. Surgical robots like the da Vinci system cost upwards of \$2 million; even research platforms like the Franka Emika Panda (\$30,000+) are prohibitively expensive for many labs. A recent wave of low-cost systems has begun to change this. ALOHA~\cite{zhao2023aloha} demonstrated fine bimanual manipulation for \$\sim$20,000 using commodity arms. GELLO~\cite{wu_2024_gello} built kinematically-matched leaders for under \$300. The Koch arm~\cite{koch2024} pushed the cost of a single leader-follower pair below \$250. These systems have enabled imitation learning at previously impossible scale.

However, all of these systems are \emph{unilateral}: the operator moves the leader, the follower tracks, and that is all. There is no force feedback channel. The operator cannot feel when the follower contacts an obstacle, grasps an object, or is overloaded. This is not merely an ergonomic gap---bilateral feedback is what made Goertz's original system safe enough for handling radioactive material, and what makes surgical telemanipulation precise enough for millimetre-scale suturing~\cite{enayati_2016_haptics, vandermeijden2009}.

The reason is straightforward: bilateral control traditionally requires force/torque sensors (\$4,000--10,000 each) or carefully calibrated dynamics models. Neither is practical on commodity servo hardware. Recent concurrent work has begun to explore this gap~\cite{echo2025, sujit_2025_improving, kobayashi2024}, but a complete, low-cost bilateral system with custom-scaled hardware and an honest characterisation of its limits has been lacking.

We present a system that closes this gap by using motor current as a force proxy. Our contributions are:

\begin{enumerate}
    \item \textbf{Sensorless bilateral force feedback} on commodity Dynamixel servos, using motor current readings as a proportional force proxy. We achieve 91.1\% obstacle detection sensitivity in a user study without any dedicated force sensors.
    ...
    \item \textbf{An empirical characterisation of the gravity-contact ambiguity}: we document four force feedback approaches, show why each fails under gravity, and establish that motor current alone cannot separate gravitational from contact forces without a dynamics model.
\end{enumerate}
...
On commodity servo hardware, dedicated force sensors are impractical. Recent concurrent work has begun exploring sensorless alternatives: GELLO+Force~\cite{sujit_2025_improving} augments GELLO with strain-gauge-based force sensing, Bi-ACT~\cite{kobayashi2024} demonstrates the importance of bilateral feedback for imitation learning quality, and Echo~\cite{echo2025} proposes open-source force feedback. Our work differs in using only the motor current readings already available on standard Dynamixel servos, requiring no additional sensing hardware, and in building custom-scaled followers rather than using commercial off-the-shelf arms.
...
\begin{equation}
    \text{PWM}_{\text{leader},j} = \begin{cases}
        \alpha \cdot (e_j - \tau) & \text{if } e_j > \tau \\
        0 & \text{otherwise}
    \end{cases}
    \label{eq:feedback}
\end{equation}

where $e_j$ is the follower effort on joint $j$, $\tau = 300$\,mA is the effort threshold (filtering noise and low-load motion), and $\alpha = 0.5$ is the proportional gain. The resulting PWM is clamped to 350 (safety cap) and exponentially smoothed with $\beta = 0.15$ before being sent to the leader motor as a resisting torque. The leader must be in PWM mode with torque enabled for this to work, which introduces electromagnetic braking as a side effect (Section~\ref{sec:gravity}).
...
\begin{table}[t]
    ...
    \begin{tabular}{@{}lcccc@{}}
        \toprule
        \textbf{Question} & \textbf{Med.} & \textbf{IQR} & \textbf{Mean} \\
        \midrule
        Q1: I could feel contact & 6 & 1.0 & 5.7 \\
        Q2: Distinguish obstacle from stiffness & 6 & 2.0 & 5.3 \\
        Q3: Feedback useful for awareness & 6 & 1.0 & 5.6 \\
        Q4: System easy to learn & \textbf{7} & 1.0 & \textbf{6.4} \\
        Q5: I felt in control & 6 & 1.0 & 5.5 \\
        \bottomrule
    \end{tabular}
\end{table}
...
Six participants achieved 100\% accuracy on all trials. The variance in specificity is explained by the gravity problem: four participants had elevated false positive rates, reporting obstacles when none were present but the arm was in an extended posture generating high gravitational effort.

\subsection{Qualitative Feedback}

Thematic analysis of open-ended responses revealed: gripper difficulty (10/22 mentions---the dominant complaint, attributable to gearbox friction rather than force feedback design), gravity/resistance confusion (6/22), positive usability (5/22: ``very intuitive,'' ``very responsive''), fear of hardware damage (2/22), and ergonomic concerns (2/22).

% ===== THE GRAVITY PROBLEM =====
\section{The Gravity-Contact Ambiguity}
\label{sec:gravity}

The central limitation of sensorless bilateral feedback on commodity hardware is that motor current conflates multiple physical phenomena: gravitational torque, contact forces, friction, inertia, and electromagnetic braking. We attempted four heuristic approaches to separate these signals; all failed, for instructive reasons.
...
\textbf{1. Simple proportional effort} (the approach we deployed): works well for obstacles at non-extreme postures but triggers false feedback when the arm is extended horizontally, where gravity alone produces 400--600\,mA at the shoulder (Fig.~\ref{fig:effort_comparison}).

\textbf{2. Velocity gating}: suppress feedback when joint velocity is near zero. This filters static gravitational loads but also removes feedback precisely at the moment of sustained contact---exactly when the operator needs it most.

\textbf{3. Position error detection}: trigger feedback when the follower cannot reach the leader's commanded position. This captures contact direction but cannot distinguish gravity-induced tracking error from obstacle-induced error.

\textbf{4. Follower velocity thresholding}: trigger feedback when the follower decelerates unexpectedly. This cannot distinguish between ``holding position under gravity'' and ``blocked by obstacle,'' as both produce near-zero follower velocity with high effort.
...
Table~\ref{tab:comparison} positions our system against related work. We are, to our knowledge, the only system that combines custom-built scaled followers, bilateral force feedback, and bimanual operation below \$5,000. ALOHA~\cite{zhao2023aloha} achieves bimanual operation but is unilateral and 7$\times$ more expensive. GELLO~\cite{wu_2024_gello} builds custom leaders but uses commercial followers and is unilateral.

\begin{table}[t]
    \centering
    \caption{Comparison with low-cost teleoperation systems.}
    \label{tab:comparison}
    \small
    \begin{tabular}{@{}lccccc@{}}
        \toprule
        \textbf{System} & \textbf{Cost} & \textbf{Bi.} & \textbf{Bim.} & \textbf{Custom} \\
        \midrule
        ALOHA~\cite{zhao2023aloha} & \$20k & \ding{55} & \ding{51} & Leaders \\
        GELLO~\cite{wu_2024_gello} & $<$\$1k & \ding{55} & \ding{51} & Leaders \\
        Koch~\cite{koch2024} & \$250 & \ding{55} & \ding{55} & Both \\
        Echo~\cite{echo2025} & --- & \ding{51} & \ding{55} & --- \\
        \textbf{Ours} & \textbf{\$3k} & \ding{51} & \ding{51} & \textbf{Followers} \\
        \bottomrule
    \end{tabular}
    \vspace{-0.5em}
    \begin{flushleft}
    \footnotesize Bi.~= Bilateral feedback. Bim.~= Bimanual. Custom~= which arms are custom-built vs. commercial off-the-shelf.
    \end{flushleft}
\end{table}

\subsection{Electromagnetic Braking Trade-off}

Enabling torque on the leader motors (required for PWM-based force feedback) introduces electromagnetic braking via slow-decay H-bridge behaviour~\cite{srikanth2008, colgate1994}. This increases the baseline resistance the operator feels, reducing backdrivability. This is a hardware-level manifestation of Lawrence's stability-transparency trade-off~\cite{lawrence_1993_stability}: more transparency (better force rendering) requires torque-enabled motors, which reduces transparency (higher baseline impedance). Resolving this likely requires series elastic actuators or lower gear-ratio motors.

\subsection{Lessons for Systems Builders}

Three practical lessons emerged:

\textbf{Integration dominates.} Individual subsystems (teleop, force feedback, collision avoidance) each worked in isolation. Combining four arms into a single ROS~2 graph exposed namespace leaks, udev conflicts, and startup race conditions that took longer to resolve than any individual feature.

\textbf{Hardware limitations propagate.} The gripper's gearbox friction (the dominant user complaint) is a mechanical property of the XL430 motor, not a software issue. No amount of algorithmic tuning can fix it. Identifying \emph{which} limitations are hardware-fundamental versus software-addressable saves engineering time.

\textbf{Negative results are load-bearing.} Our four failed force feedback approaches each reveal a different facet of the gravity-contact ambiguity. Documenting \emph{why} they fail provides more actionable guidance than documenting only the approach that worked.

% ===== CONCLUSION =====
\section{Conclusion}

We presented a low-cost bilateral teleoperation system that achieves sensorless force feedback using motor current readings from commodity Dynamixel servos. The system pairs standard PincherX-100 leaders with custom 2$\times$-scaled 3D-printed followers using a torque-tiered motor design, achieving bimanual bilateral teleoperation at $\sim$\$3,000---an order of magnitude cheaper than comparable systems. A user study demonstrated 91.1\% obstacle detection sensitivity and strong perceived learnability. We characterised the fundamental gravity-contact ambiguity that limits sensorless bilateral control and documented four approaches that fail to resolve it, establishing that a dynamics model is a necessary condition for robust force estimation on commodity hardware.
